...
class TestPhoneTable
{
	...
	public function fetchAll($array=FALSE)
	{
		...
		$orderCol = $platform->quoteIdentifier('test_phone_id');
	...
	public function fetchAllTestPhone(array $columns = NULL, array $joins = NULL, array $literals = NULL, $limit = NULL, $group = NULL, $order = NULL, $buffer = FALSE, $having = NULL)
	{
		$platform = $this->getAdapter()->getPlatform();
		...
		$setstmt = $this->getAdapter()->createStatement("set @test_phone_row_num = 0");
		$set = $setstmt->execute();
	...
	public function getTestPhone($id)
	{
		$id  = (int) $id;
		$rowset = $this->tableGateway->select(array('test_phone_id' => $id));
		$row = $rowset->current();
		if (!$row) {
	...
	public function getTestPhoneObject($id)
	{
		$id  = (int) $id;
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
		$Col = $platform->quoteIdentifier('test_phone_id');
	...
	public function saveTestPhone(TestPhone $testphone)
	{
		$data = array(
			'test_phone_id'=>$testphone->test_phone_id,
			'phone_number'=>$testphone->phone_number,
			);

			$id = (int)$testphone->test_phone_id;
			if ($id == 0) {
				$this->tableGateway->insert($data);
				return $this->tableGateway->lastInsertValue;
		} else {
			if ($this->getTestPhone($id)) {
				$this->tableGateway->update($data, array('test_phone_id' => $id));
			} else {
			throw new RuntimeException(sprintf(
			        'Could not find row with identifier %d',
			        $id
	...
	public function updateTestPhone($data,$id)
	{
		$this->tableGateway->update($data, array('test_phone_id' => $id));
	}

	public function conditionalUpdateTestPhone($data,array $condition)
	{
		$this->tableGateway->update($data,$condition);
	}

	public function deleteTestPhone($id)
	{
		$this->tableGateway->delete(array('test_phone_id' => $id));
	}
	...
	public function getTestPhoneCol($Col)
	{
		$platform = $this->getAdapter()->getPlatform();
		$Col = $platform->quoteIdentifier($Col); 
		$id = $platform->quoteIdentifier('test_phone_id');
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function getTestPhoneRecordsCount()
	{
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function fetchTestPhoneByCol($searchTerm,$WhereCol)
	{
		$platform = $this->getAdapter()->getPlatform();
		$WhereCol = $platform->quoteIdentifier($WhereCol);
		$table = $platform->quoteIdentifier($this->getTable());
