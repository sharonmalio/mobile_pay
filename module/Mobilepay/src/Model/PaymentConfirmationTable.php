...
class PaymentConfirmationTable
{
	...
	public function fetchAll($array=FALSE)
	{
		...
		$orderCol = $platform->quoteIdentifier('payment_confirmation_id');
	...
	public function fetchAllPaymentConfirmation(array $columns = NULL, array $joins = NULL, array $literals = NULL, $limit = NULL, $group = NULL, $order = NULL, $buffer = FALSE, $having = NULL)
	{
		$platform = $this->getAdapter()->getPlatform();
		...
		$setstmt = $this->getAdapter()->createStatement("set @payment_confirmation_row_num = 0");
		$set = $setstmt->execute();
	...
	{
		$id  = (int) $id;
		$rowset = $this->tableGateway->select(array('payment_confirmation_id' => $id));
		$row = $rowset->current();
		if (!$row) {
	...
	public function getPaymentConfirmationObject($id)
	{
		$id  = (int) $id;
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
		$Col = $platform->quoteIdentifier('payment_confirmation_id');
	...
	public function savePaymentConfirmation(PaymentConfirmation $paymentconfirmation)
	{
		$data = array(
			'appnt_payment_confirmation_id'=>$paymentconfirmation->appnt_payment_confirmation_id,
			'merchant_request_id'=>$paymentconfirmation->merchant_request_id,
			'checkout_request_id'=>$paymentconfirmation->checkout_request_id,
			'result_code'=>$paymentconfirmation->result_code,
			'result_desc'=>$paymentconfirmation->result_desc,
			'service_cost'=>$paymentconfirmation->service_cost,
			'mpesa_transaction_id'=>$paymentconfirmation->mpesa_transaction_id,
			'mpesa_transaction_date'=>$paymentconfirmation->mpesa_transaction_date,
			'appnt_user_phone_number'=>$paymentconfirmation->appnt_user_phone_number,
			);

			$id = (int)$paymentconfirmation->payment_confirmation_id;
			if ($id == 0) {
				$this->tableGateway->insert($data);
				return $this->tableGateway->lastInsertValue;
		} else {
			if ($this->getPaymentConfirmation($id)) {
				$this->tableGateway->update($data, array('payment_confirmation_id' => $id));
			} else {
			throw new RuntimeException(sprintf(
			        'Could not find row with identifier %d',
			        $id
	...
	public function updatePaymentConfirmation($data,$id)
	{
		$this->tableGateway->update($data, array('payment_confirmation_id' => $id));
	}

	public function conditionalUpdatePaymentConfirmation($data,array $condition)
	{
	...
	{
		$this->tableGateway->delete(array('payment_confirmation_id' => $id));
	}
	...
	public function getPaymentConfirmationCol($Col)
	{
		$platform = $this->getAdapter()->getPlatform();
		$Col = $platform->quoteIdentifier($Col); 
		$id = $platform->quoteIdentifier('payment_confirmation_id');
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function getPaymentConfirmationRecordsCount()
	{
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function fetchPaymentConfirmationByCol($searchTerm,$WhereCol)
	{
		$platform = $this->getAdapter()->getPlatform();
		$WhereCol = $platform->quoteIdentifier($WhereCol);
		$table = $platform->quoteIdentifier($this->getTable());
